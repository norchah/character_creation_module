...
def attack(char_name, char_class):
    war_damage = 5 + randint(3, 5)
    mage_damage = 5 + randint(5, 10)
    heal_damage = 5 + randint(-3, -1)
    if char_class == "warrior":
        return f"{char_name} нанёс урон противнику равный {war_damage}"
    if char_class == "mage":
        return f"{char_name} нанёс урон противнику равный {mage_damage}"
    if char_class == "healer":
        return f"{char_name} нанёс урон противнику равный {heal_damage}"
...
def choice_char_class():
    ...
    while approve_choice != "y":
        ...
        approve_choice = input(
            "Нажми (Y), чтобы подтвердить выбор, или любую другую кнопку, чтобы выбрать другого персонажа "
        ).lower()
    return char_class
